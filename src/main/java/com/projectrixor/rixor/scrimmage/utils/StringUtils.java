...
public class StringUtils {
     ...
     *
     * @see StringUtils#listToEnglishCompound
     */
    ...
    public static String formatHeader(String header) {
        ChatColor dashColor = ChatColor.DARK_PURPLE;
        ChatColor textColor = ChatColor.GOLD;
        ChatColor highlight = ChatColor.YELLOW;

        String message = textColor + header + textColor + " (" + highlight + "[page]" + textColor + " of " + highlight + "[pages]" + textColor + ")";
        return padMessage(message, "-", dashColor);
    }
    ...
    public static Color convertHexStringToColor(String string) {
        if(string == null) return null;
        if(!string.substring(0, 1).equals("#")) string = "#" + string;
    ...
    public static final <T> T bestFuzzyMatch(String search, Collection<T> options, double threshold) {
        T bestObj = null;
